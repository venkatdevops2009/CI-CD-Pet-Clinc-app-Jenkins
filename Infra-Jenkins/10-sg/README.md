...
- One SG per name in `var.sg_names` ([variables.tf](./variables.tf)), using the
  reusable [`terraform-aws-sg`](https://github.com/venkatdevops2009/terraform-aws-sg.git)
...
- One SSM parameter per SG holding its ID ([parameters.tf](./parameters.tf)):
  `/petclinc/dev/<name>_sg_id` (e.g. `/petclinc/dev/mongodb_sg_id`).

## Depends on
- `00-vpc` → reads `/petclinc/dev/vpc_id` ([data.tf](./data.tf)).
